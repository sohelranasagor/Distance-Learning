<?php
    ...
    if($_SERVER['REQUEST_METHOD']=="POST"){
        ...
        $coursetype=$_SESSION['typeid'];
        $instructor=$result['facultyname'];
        $courselevel=$_POST['courseLavel'];
        $info = pathinfo($_FILES['coursepic']['name']);

        // $ext = $info['extension'];
        $newname = $_FILES['coursepic']['name'];

        $target = 'coursepic/' . $newname;

        move_uploaded_file($_FILES['coursepic']["tmp_name"], $target);

        $result1=addNewCourses($courseTitle,$courseDescription,$target,$courseFeature,$instructor,$courseNumber,$coursetype,$courselevel);
        if($result1)
        {
            $result2=loadCourse($courseNumber);
            $_SESSION['courseId']=$result2['courseId'];
			...
			<table border="0" width="100%">
				...
				<td>
                    ...
                    <tr>
                        ...
                        <td>
                        <input type="text" name="courseTitle" placeholder="" />
                        </td>
                        </tr>
                        <tr> <!-- create a table row -->
                            <td align="right">Course Description:</td> <!-- set table data -->
                            <td><input type="text" name="courseDescription" placeholder="" /></td>
                        </tr>

                        <tr>
                            <td align="right">Course Picture:</td>
                            <td><input type="file" name="coursepic" multiple></td>
                        </tr>
                        <tr>
                            <td align="right">Course Feature:</td>
                            <td><input type="text" name="courseFeature" /></td>
                        </tr>
                        <tr>
